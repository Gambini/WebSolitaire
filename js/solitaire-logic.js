...
sol.update = function() {
	if(sol.state.drag)
	return;
	//this is the stuid and dirty way of making sure that stacks don't move.
	var stack_spacing = 20;
	var off = [CardMetrics.middle[0], CardMetrics.dim.h + stack_spacing + CardMetrics.middle[1]];
	for (var i = 0; i < 8; ++i) {
	if(sol.scratch[i].cards.length > 0) {
		//make sure that the bottom card is where it should be
		var deck = sol.scratch[i];
		var cv = deck.cards[0].rect.GetTransformedVerts();
		var dv = deck.rect.GetTransformedVerts();
		if(cv[1] != dv[1]) { // if the tops don't match
		deck.rect.MoveTo(off[0],off[1]);
		deck.cards[0].rect.MoveTo(off[0],off[1]);
		deck.SpreadAllCards();
		}
	}
	else {
		sol.scratch[i].rect.MoveTo(off[0],off[1]);
	}
	off[0] += CardMetrics.dim.w + stack_spacing;
	}
